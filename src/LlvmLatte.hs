...
instance Show LlvmInst where
  show inst = unwords parts where
    parts = case inst of
      ...
      (Br val lT lF) -> [sep, "br", unwordsSep [show val, label lT, label lF]]
      (Goto l) -> [sep, "br", label l]
      (Lab l) -> [tail l ++ ":"]
      ...
      (StrLit loc s sLoc) -> [sep, loc, "=", getelementptrStr s sLoc]
...
getelementptrStr :: String -> Loc -> String
getelementptrStr s sLoc =
...
data BasicBlock = BasicBlock Label [LlvmInst]  -- todo?
